...
@Entity //сущность
@Table(name = "Player") //из таблицы
public class Player {
    @Id //id
    @Column(name = "id") // из столбца
    @GeneratedValue(strategy = GenerationType.IDENTITY) //герерируется на стороне БД как id
    private Long id;
    @Column(name = "name")
    private String name;
    @Column(name = "title")
    ...
    @Enumerated(EnumType.STRING)// spring будет знать, что значения берутся из списка по названию
